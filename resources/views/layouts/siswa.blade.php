...
<body class="bg-gray-50 font-['Inter']"
    ...
    <div class="flex flex-col h-screen overflow-hidden">
            ...
            <div class="flex flex-col h-full">
                ...
                <div class="px-4 py-4 border-b border-gray-100">
                    <div class="flex items-center space-x-3 px-3 py-3 bg-linear-to-r from-blue-50 to-indigo-50 rounded-2xl border border-blue-100/50">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-blue-600 font-medium truncate">{{ Auth::user()->class->name ?? 'Belum ada kelas' }}</p>
                        </div>
                    </div>
                </div>
                ...
                <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto scrollbar-hide">
                    ...
                    <a href="{{ route('siswa.attendance.index') }}"
                       class="flex items-center px-4 py-3 text-gray-700 rounded-xl hover:bg-blue-50 hover:text-blue-600 transition-all group {{ request()->routeIs('siswa.attendance.*') ? 'bg-blue-50 text-blue-600' : '' }}">
                        ...
                        <span class="ml-3 font-medium">{{ __('messages.my_attendance') }}</span>
                        @if(request()->routeIs('siswa.attendance.*'))
                            <span class="ml-auto w-1.5 h-8 bg-blue-600 rounded-full"></span>
                        @endif
                    </a>
                    <a href="{{ route('siswa.daily-attendance.index') }}"
                       class="flex items-center px-4 py-3 text-gray-700 rounded-xl hover:bg-blue-50 hover:text-blue-600 transition-all group {{ request()->routeIs('siswa.daily-attendance.*') ? 'bg-blue-50 text-blue-600' : '' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                        </svg>
                        <span class="ml-3 font-medium">Kehadiran Saya</span>
                        @if(request()->routeIs('siswa.daily-attendance.*'))
                            <span class="ml-auto w-1.5 h-8 bg-blue-600 rounded-full"></span>
                        @endif
                    </a>
                    @endfeature
                    ...
                    <a href="{{ route('siswa.grades.index') }}"
                       class="flex items-center px-4 py-3 text-gray-700 rounded-xl hover:bg-blue-50 hover:text-blue-600 transition-all group {{ request()->routeIs('siswa.grades.*') ? 'bg-blue-50 text-blue-600' : '' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="p-4 border-t border-gray-100">
                    <div class="flex items-center space-x-3 px-3 py-3 rounded-2xl hover:bg-gray-50 transition-colors">
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-bold text-gray-900 truncate">{{ Auth::user()->name }}</p>
                            <p class="text-xs text-gray-400 font-medium">Siswa</p>
                        </div>
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            {{-- Topbar --}}
            <header class="bg-white/80 backdrop-blur-xl border-b border-gray-100 sticky top-0 z-80">
                        ...
                        <div class="flex items-center gap-2 lg:gap-4">
                            ...
                            <div class="relative" x-data="{ profileOpen: false }">
                                ...
                                <div x-show="profileOpen" @click.away="profileOpen = false"
                                     ...
                                     class="hidden lg:block absolute right-0 mt-3 w-64 bg-white rounded-2xl shadow-xl border border-gray-100 z-90 overflow-hidden">
                                    <div class="px-5 py-4 bg-linear-to-r from-blue-50 to-indigo-50 border-b border-gray-100">
                                        <div class="flex items-center space-x-3">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <p class="text-sm font-bold text-gray-900 truncate">{{ Auth::user()->name }}</p>
                                                <p class="text-xs text-blue-600 font-medium">{{ Auth::user()->class->name ?? '-' }}</p>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="py-2">
                                        <a href="{{ route('siswa.profile') }}" class="flex items-center gap-3 px-5 py-2.5 text-sm font-medium text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition-colors">
                                        ...
                                        @feature('nilai_tugas')
                                        <a href="{{ route('siswa.grades.index') }}" class="hidden lg:flex items-center gap-3 px-5 py-2.5 text-sm font-medium text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition-colors lg:hidden">
                                        ...
                                        <a href="{{ route('sekretaris.dashboard') }}" class="flex items-center gap-3 px-5 py-2.5 text-sm font-medium text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition-colors">
                                            <div class="w-8 h-8 bg-indigo-100 rounded-lg flex items-center justify-center">
                                                <svg class="w-4 h-4 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7v8a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2M8 7H6a2 2 0 00-2 2v6a2 2 0 002 2h2m4-10h2a2 2 0 012 2v6a2 2 0 01-2 2h-2"></path>
                                                </svg>
                                            </div>
                                        ...
                                        <div class="relative w-full bg-white rounded-t-3xl safe-bottom max-h-[85vh] overflow-y-auto animate-slide-up">
                                            ...
                                            <div class="px-6 pb-4 flex items-center gap-4 border-b border-gray-100">
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    ...
                                                    <p class="text-sm text-blue-600 font-medium">{{ Auth::user()->class->name ?? '-' }}</p>
                                                    <p class="text-xs text-gray-400 mt-0.5">NIS: {{ Auth::user()->nis ?? '-' }}</p>
                                                </div>
                                            </div>
                                            {{-- Actions --}}
                                            <div class="py-3 px-4 space-y-1">
                                                <a href="{{ route('siswa.profile') }}" class="flex items-center gap-4 px-4 py-3 rounded-2xl hover:bg-gray-50 transition-colors">
                                                    <div class="w-10 h-10 bg-blue-100 rounded-xl flex items-center justify-center">
                                                        <svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                                                    </div>
                                                    <div class="flex-1">
                                                        <p class="text-sm font-semibold text-gray-900">Lihat Profil</p>
                                                ...
                                                @feature('nilai_tugas')
                                                <a href="{{ route('siswa.grades.index') }}" class="flex items-center gap-4 px-4 py-3 rounded-2xl hover:bg-gray-50 transition-colors">
                                                    ...
                                                    <div class="flex-1">
                                                        <p class="text-sm font-semibold text-gray-900">Nilai Tugas</p>
                                                        <p class="text-xs text-gray-500">Lihat nilai tugas per mapel</p>
                                                    </div>
                                                    <svg class="w-5 h-5 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                                                </a>
                                                @endfeature
                                                @if(Auth::user()->hasRole('sekretaris'))
                                                <a href="{{ route('sekretaris.dashboard') }}" class="flex items-center gap-4 px-4 py-3 rounded-2xl hover:bg-gray-50 transition-colors">
                                                    ...
                                                    <div class="flex-1">
                                                        <p class="text-sm font-semibold text-gray-900">Portal Sekretaris</p>
                                                        <p class="text-xs text-gray-500">Beralih ke portal sekretaris</p>
                                                    </div>
                ...
                <div class="flex items-stretch">
                    @php
                        $navItems = [
                            ...
                            [
                                'route' => 'siswa.agenda.index',
                                'active' => request()->routeIs('siswa.agenda.*'),
                                'label' => 'Agenda',
                            ...
                            [
                                'route' => 'siswa.schedule.index',
                                'active' => request()->routeIs('siswa.schedule.*'),
                                'label' => 'Jadwal',
                                'icon_outline' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3.75 6A2.25 2.25 0 016 3.75h2.25A2.25 2.25 0 0110.5 6v2.25a2.25 2.25 0 01-2.25 2.25H6a2.25 2.25 0 01-2.25-2.25V6zM3.75 15.75A2.25 2.25 0 016 13.5h2.25a2.25 2.25 0 012.25 2.25V18a2.25 2.25 0 01-2.25 2.25H6A2.25 2.25 0 013.75 18v-2.25zM13.5 6a2.25 2.25 0 012.25-2.25H18A2.25 2.25 0 0120.25 6v2.25A2.25 2.25 0 0118 10.5h-2.25a2.25 2.25 0 01-2.25-2.25V6zM13.5 15.75a2.25 2.25 0 012.25-2.25H18a2.25 2.25 0 012.25 2.25V18A2.25 2.25 0 0118 20.25h-2.25A2.25 2.25 0 0113.5 18v-2.25z"></path>',
                                'icon_filled' => '<path fill-rule="evenodd" d="M3 6a3 3 0 013-3h2.25a3 3 0 013 3v2.25A3 3 0 018.25 11.25H6A3 3 0 013 8.25V6zm9.75 0A3 3 0 0115.75 3H18a3 3 0 013 3v2.25a3 3 0 01-3 3h-2.25a3 3 0 01-3-3V6zM3 15.75A3 3 0 016 12.75h2.25a3 3 0 013 3V18a3 3 0 01-3 3H6a3 3 0 01-3-3v-2.25zm9.75 0a3 3 0 013-3H18a3 3 0 013 3V18a3 3 0 01-3 3h-2.25a3 3 0 01-3-3v-2.25z" clip-rule="evenodd"></path>',
                                'guard' => 'schedule',
                            ],
                            [
                                'route' => 'siswa.attendance.index',
                                'active' => request()->routeIs('siswa.attendance.*'),
                                'label' => 'Presensi',
                            ...
                            [
                                'route' => 'siswa.daily-attendance.index',
                                'active' => request()->routeIs('siswa.daily-attendance.*'),
                                'label' => 'Kehadiran',
                    ...
                    @foreach($navItems as $item)
                        @if(empty($item['guard']) || \App\Services\FeatureService::isEnabled($item['guard']))
