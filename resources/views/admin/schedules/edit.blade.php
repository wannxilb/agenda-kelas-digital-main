        ...
        <form action="{{ route('admin.schedules.update', $schedule) }}" method="POST">
            ...
            <div class="p-8 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select name="week_type" id="week_type" required
                            ...
                            <option value="semua" {{ old('week_type', $schedule->week_type) == 'semua' ? 'selected' : '' }}>Semua Minggu (Tiap Minggu)</option>
                            <option value="ganjil" {{ old('week_type', $schedule->week_type) == 'ganjil' ? 'selected' : '' }}>Minggu Ganjil (Ke-1, 3, dst)</option>
                            <option value="genap" {{ old('week_type', $schedule->week_type) == 'genap' ? 'selected' : '' }}>Minggu Genap (Ke-2, 4, dst)</option>
                        </select>
                    ...
                    <!-- Jumlah JP -->
                    @php
                        $defaultJp = old('jumlah_jp');
                        if (!$defaultJp) {
                            $diff = strtotime($schedule->end_time) - strtotime($schedule->start_time);
                            $minutes = $diff > 0 ? round($diff / 60) : $schedule->subject->credit_hours ?? 2;
                            $defaultJp = max(1, round($minutes / 45));
                        }
                    @endphp
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Jumlah JP <span class="text-red-500">*</span></label>
                        <input type="number" name="jumlah_jp" id="jumlah_jp" min="1" value="{{ $defaultJp }}" required
                ...
                <div class="bg-blue-50/50 rounded-xl p-4 border border-blue-100 flex flex-col md:flex-row justify-between text-sm text-blue-800">
                    <div><span class="font-semibold">Dibuat:</span> {{ $schedule->created_at->format('d/m/Y H:i') }}</div>
                    <div><span class="font-semibold">Diperbarui:</span> {{ $schedule->updated_at->format('d/m/Y H:i') }}</div>
                </div>
                @endif
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateTeacherSelect(subjectId) {
            var prevValue = teacherTom.getValue();
            teacherTom.clear();
            teacherTom.clearOptions();

            if (subjectId && subjectTom && subjectTom.options[subjectId]) {
                var teachers = subjectTom.options[subjectId].teachers ?? [];
        ...
        async function loadRooms() {
            ...
            var prevValue = roomTom.getValue() || '{{ $schedule->room_id }}';
            ...
            try {
                var response = await fetch('{{ route('admin.schedules.get-available-rooms') }}?day=' + day + '&start_time=' + start + '&end_time=' + end + '&schedule_id=' + currentScheduleId);
