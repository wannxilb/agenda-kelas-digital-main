...
@section('content')
@php
    $hasActiveFilters = request()->anyFilled(['search', 'date']);
@endphp
<div class="space-y-3 sm:space-y-5 pb-24 sm:pb-6"
    ...
    {{-- Search & Filter Bar --}}
    <div class="relative z-10 -mx-3 sm:mx-0 px-3 sm:px-0 pb-1 sm:pb-0 transition-all duration-200"
        ...
        <div class="flex items-center justify-between gap-3 mb-3" x-show="!scrolled" x-collapse.duration.150ms>
            <div>
                <p class="text-xs sm:text-sm text-gray-500 leading-relaxed">
                    Lihat daftar agenda harian, ringkasan materi, dan tugas kelas.
                </p>
            </div>
        </div>

        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden sm:overflow-visible">
            <form method="GET" action="{{ route('siswa.agenda.index') }}" id="filterForm" x-ref="filterForm" class="sm:relative">
                ...
                <div class="p-3 sm:p-4 flex items-center gap-2">
                    <div class="relative flex-1">
                        ...
                        @if(request('search'))
                            <button type="button"
                                    onclick="document.querySelector('input[name=search]').value=''; document.getElementById('filterForm').requestSubmit();"
                    ...
                    <button type="button" @click="showFilter = !showFilter"
                        ...
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                         ...
                         @if(request()->anyFilled(['date', 'search']))
                ...
                <div x-show="showFilter" x-collapse x-cloak
                     class="border-t border-gray-100 bg-gray-50/50 px-3 sm:px-4 py-3 space-y-3 sm:absolute sm:top-full sm:right-0 sm:mt-2 sm:w-90 sm:border sm:border-gray-200 sm:bg-white sm:rounded-2xl sm:shadow-xl sm:z-50">
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5 px-1">Pilih Tanggal</label>
    ...
    <div class="flex items-center justify-between px-0.5">
        <h3 class="text-sm font-bold text-gray-800">Daftar Agenda</h3>
        <span class="text-[10px] font-bold text-gray-400 bg-gray-100 px-2.5 py-1 rounded-lg uppercase tracking-wider">{{ $agendas->total() }} agenda</span>
    </div>
    ...
    @if($agendas->isEmpty())
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-10 sm:p-16 text-center">
            ...
            <p class="text-xs sm:text-sm text-gray-500 mt-1 max-w-xs mx-auto">
                ...
                @else
                    Belum ada agenda yang dipublikasikan untuk kelas ini.
                @endif
            </p>
            @if($hasActiveFilters)
                <a href="{{ route('siswa.agenda.index') }}" class="mt-4 inline-flex items-center gap-1.5 px-4 py-2.5 bg-gray-100 text-gray-700 text-xs font-bold rounded-xl hover:bg-gray-200 active:scale-95 transition-all">
                    Reset Filter
                </a>
            @endif
        </div>
    @else
        <div class="space-y-6">
            @foreach($groupedAgendas as $date => $dailyAgendas)
                ...
                <div class="sticky top-26 sm:static z-20 -mx-3 sm:mx-0 px-3 sm:px-0 py-1.5 sm:py-0 bg-gray-50/95 sm:bg-transparent backdrop-blur-sm sm:backdrop-blur-none flex items-center gap-3">
                    <div class="flex items-center gap-2">
                        ...
                        @else
                            @php
                                ...
                                [$y, $m, $d] = explode('-', $date);
                                $formattedDate = (int)$d . ' ' . ($months[(int)$m] ?? $m) . ' ' . $y;
                            @endphp
                    ...
                    <span class="text-[10px] font-semibold text-gray-400">{{ count($dailyAgendas) }} agenda</span>
                </div>
                    ...
                    @foreach($dailyAgendas as $agenda)
                        ...
                        <div class="agenda-card bg-white rounded-2xl shadow-sm border border-gray-100 border-l-[3px] {{ $c['border'] }} overflow-hidden active:scale-[0.98] hover:shadow-md transition-all duration-200 cursor-pointer group"
                            ...
                            <div class="p-3.5 sm:p-5">
                                {{-- Top Row: Subject badge + Attachment --}}
                                ...
                                <div class="mt-3 flex min-w-0 items-center justify-between">
                                    <div class="flex flex-1 min-w-0 items-center gap-2 overflow-hidden">
                                        <span class="inline-flex min-w-0 max-w-[42%] items-center gap-1 text-[10px] sm:max-w-none sm:text-[11px] text-gray-400 font-medium">
                                            <svg class="w-3 h-3 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                                            </svg>
                                            <span class="min-w-0 truncate">{{ $agenda->teacher->name ?? '-' }}</span>
                                        </span>
                                        @if($agenda->room)
                                        <span class="inline-flex min-w-0 max-w-[38%] items-center gap-1 text-[10px] sm:max-w-none sm:text-[11px] text-gray-400 font-medium">
                                            <svg class="w-3 h-3 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        @endif
                                        <span class="inline-flex min-w-0 max-w-[20%] items-center gap-1 text-[10px] sm:max-w-none sm:text-[11px] text-gray-400 font-medium">
                                            <svg class="w-3 h-3 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @if($agendas->hasPages())
            <div class="flex justify-center">
                {{ $agendas->appends(request()->except('status'))->links() }}
            </div>
        @endif
    @endif
</div>
...
<button x-data="{ show: false }"
        ...
        x-transition
        class="sm:hidden fixed bottom-24 right-5 z-40 w-10 h-10 flex items-center justify-center bg-white text-gray-500 rounded-xl shadow-lg border border-gray-100 active:scale-90 transition-transform"
        aria-label="Ke atas">
...
<div x-data="agendaModal()" x-cloak x-show="open"
    ...
    <div class="fixed inset-x-0 bottom-0 sm:inset-0 sm:flex sm:items-center sm:justify-center sm:p-4 z-10">
        <div class="bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl w-full sm:max-w-lg max-h-[92vh] sm:max-h-[85vh] overflow-hidden flex flex-col"
             @click.outside="close()"
             x-ref="sheet"
            ...
            <div class="relative bg-linear-to-br from-blue-600 via-indigo-600 to-indigo-700 px-5 sm:px-6 py-5 sm:py-6 overflow-hidden shrink-0">
                ...
                <div class="absolute -bottom-6 -left-6 w-20 h-20 bg-white/10 rounded-full blur-2xl"></div>
                <button type="button" @click="close()" class="absolute top-3 right-3 w-9 h-9 flex items-center justify-center bg-white/20 hover:bg-white/30 text-white rounded-xl active:scale-90 transition-all z-10">
                    ...
                    <template x-if="!loading">
                        <div>
                            <div class="flex items-center gap-2 mb-2">
                                <span class="inline-flex items-center px-2.5 py-0.5 bg-white/15 rounded-lg text-[10px] font-bold uppercase tracking-wider text-white/90 ring-1 ring-white/10"
                                      x-text="agenda.subject_name || 'Umum'"></span>
                            </div>
                            <h3 class="text-lg sm:text-xl font-bold text-white leading-snug" x-text="agenda.title"></h3>
                        </div>
                    </template>
                </div>
            </div>
            ...
            {{-- Button area dengan spacing yang diperbaiki --}}
            <div class="border-t border-gray-200 px-5 sm:px-6 py-5 sm:py-6 bg-gray-50 safe-bottom shrink-0">
                <div class="flex gap-3">
                    <button @click="close()" class="flex-1 py-3.5 sm:py-3.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl text-sm font-bold transition-colors active:scale-[0.98]">
                        Tutup
                    </button>
                </div>
            </div>
            ...
            get weekDays() {
                ...
                for (let i = 0; i < 7; i++) {
                    ...
                    result.push({
                        num: d.getDate(),
                        date: dateStr,
                        shortLabel: shortDays[d.getDay()] + ' ' + d.getDate(),
                        hasAgenda: this.agendaDates.includes(dateStr)
                    });
                }
                return result;
