...
<div class="space-y-4 sm:space-y-6 pb-8 max-w-2xl mx-auto">
    ...
    @if(session('success'))
        <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 3000)"
             x-transition class="bg-emerald-50 border border-emerald-200 rounded-2xl px-4 py-3 flex items-center gap-3">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <form action="{{ route('sekretaris.profile.update') }}" method="POST" class="p-5 space-y-4">
            @csrf
            <div>
