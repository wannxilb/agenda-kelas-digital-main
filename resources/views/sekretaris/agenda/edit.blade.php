...
<div class="max-w-5xl mx-auto pb-24 sm:pb-12 px-2 sm:px-0">
    <div class="mb-6 sm:mb-8 flex flex-col sm:block mt-2 sm:mt-0 px-2 sm:px-0">
        <div class="flex items-center gap-3 sm:hidden mb-1">
            ...
            <h1 class="text-xl font-black text-gray-900 tracking-tight">Edit Agenda</h1>
        </div>
        ...
        <nav class="hidden sm:flex mb-4 text-xs font-bold uppercase tracking-widest text-gray-400">
            <ol class="flex items-center space-x-2">
                ...
                <li><svg class="w-3 h-3 mx-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg></li>
                <li class="text-indigo-600">Edit Agenda</li>
            </ol>
        </nav>
        <h1 class="hidden sm:block text-3xl font-black text-gray-900 tracking-tight">Edit Agenda</h1>
        <p class="hidden sm:block text-gray-500 mt-1 font-medium text-sm">Sesuaikan informasi agenda yang telah dicatat sebelumnya.</p>
    </div>
            ...
            <div class="p-5 sm:p-8 space-y-6">
                <div class="space-y-2">
                    ...
                    <input type="text" name="title" value="{{ old('title', $agenda->title) }}" required
                           class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all text-sm @error('title') border-red-500 @enderror"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="date" id="date" value="{{ old('date', \Carbon\Carbon::parse($agenda->date)->format('Y-m-d')) }}" required
                               class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all text-sm @error('date') border-red-500 @enderror">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <select name="subject_id" id="subject_id" class="w-full text-sm">
                            ...
                            @foreach($subjects as $subject)
                                <option value="{{ $subject->id }}"
                                    ...
                                    {{ old('subject_id', $agenda->subject_id) == $subject->id ? 'selected' : '' }}>
                                    {{ $subject->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <p id="room-hint" class="text-[10px] font-semibold text-indigo-500 mt-1 {{ $scheduleRooms->isNotEmpty() ? '' : 'hidden' }}">
                        ...
                        <p id="room-fallback-hint" class="text-[10px] font-semibold text-amber-500 mt-1 {{ $scheduleRooms->isEmpty() ? '' : 'hidden' }}">
                    ...
                    <div>
                        ...
                        <select name="teacher_id" id="teacher_id" required
                            ...
                            @foreach($scheduleTeachers as $t)
                                <option value="{{ $t['id'] }}" {{ old('teacher_id', $agenda->teacher_id) == $t['id'] ? 'selected' : '' }}>
                                    {{ $t['name'] }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="p-5 sm:p-8 space-y-6">
                <div>
                    ...
                    <div class="rounded-xl border border-gray-200 overflow-hidden bg-white">
                        <div id="editor" class="min-h-[150px] sm:min-h-[250px] bg-white"></div>
                    </div>
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Lampiran Dokumen (Kosongkan jika tidak ingin mengubah)</label>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function rebuildOptions(selectId, options) {
        var tom = document.getElementById(selectId).tomselect;
        if (!tom) return;
        var prevVal = tom.getValue();
        tom.close();
        tom.clearOptions();
        options.forEach(function(opt) { tom.addOption(opt); });
        tom.refreshOptions(false);
        if (prevVal) {
            tom.setValue(prevVal, true);
        }
        tom.close();
    }
    ...
    function fetchScheduleInfo(date) {
        ...
        fetch(API_URL + '?' + params.toString())
            ...
            .then(function(data) {
                ...
                rebuildOptions('subject_id', subjOpts);
                rebuildOptions('room', roomOpts);
                rebuildOptions('teacher_id', teacherOpts);
                ...
                var curSubj = subjectTom.getValue();
                if (curSubj) applySubjectFilter(curSubj);
            })
            .catch(function() {});
    }
    ...
    subjectTom.on('change', function(value) {
