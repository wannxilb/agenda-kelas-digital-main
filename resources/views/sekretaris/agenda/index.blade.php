...
<div class="space-y-3 sm:space-y-5 pb-24 sm:pb-6"
    ...
    <div class="relative z-30 -mx-3 sm:mx-0 px-3 sm:px-0 pb-1 sm:pb-0 transition-all duration-200"
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden sm:overflow-visible">
            <form method="GET" action="{{ route('sekretaris.agenda.index') }}" id="filterForm" x-ref="filterForm" class="sm:relative">
                ...
                {{-- Quick status chips --}}
                <div class="px-3 sm:px-4 pb-3 flex items-center gap-2 overflow-x-auto scrollbar-hide">
                    @php
                        $chip = function($value, $label) {
                            $active = request('status') === $value || ($value === 'all' && !request('status'));
                    ...
                    @endphp
                    <a href="{{ route('sekretaris.agenda.index', array_merge(request()->except('page'), ['status' => 'all'])) }}"
                       class="{{ $chip('all', 'Semua')[0] }} {{ !request('status') ? '' : '' }}">Semua</a>
                    <a href="{{ route('sekretaris.agenda.index', array_merge(request()->except('page'), ['status' => 'published'])) }}"
                       class="{{ $chip('published', 'Published')[0] }}">Published</a>
                    <a href="{{ route('sekretaris.agenda.index', array_merge(request()->except('page'), ['status' => 'draft'])) }}"
                       class="{{ $chip('draft', 'Draft')[0] }}">Draft</a>
                    @if(request()->anyFilled(['search', 'date', 'status']))
                        <a href="{{ route('sekretaris.agenda.index') }}"
                           class="shrink-0 inline-flex items-center gap-1 px-2.5 py-1.5 text-[11px] font-bold text-red-600 bg-red-50 rounded-full">
    ...
    {{-- Agenda Count --}}
    <div class="flex items-center justify-between px-1.5 !mt-1 mb-2">
    ...
    @if($agendas->isEmpty())
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-10 sm:p-16 text-center">
            ...
            <p class="text-xs sm:text-sm text-gray-500 mt-1 max-w-xs mx-auto">
                ...
                @else
                    Mulai buat agenda harian untuk kelasmu.
                @endif
            </p>
            @if(request()->anyFilled(['search','date','status']))
                <a href="{{ route('sekretaris.agenda.index') }}" class="mt-4 inline-flex items-center gap-1.5 px-4 py-2.5 bg-gray-100 text-gray-700 text-xs font-bold rounded-xl hover:bg-gray-200 active:scale-95 transition-all">
                    Reset Filter
                </a>
            @endif
        </div>
                    ...
                    @foreach($dailyAgendas as $agenda)
                        ...
                        <div class="agenda-card bg-white rounded-2xl shadow-sm border border-gray-100 border-l-[3px] {{ $c['border'] }} overflow-hidden active:scale-[0.98] hover:shadow-md transition-all duration-200 cursor-pointer group"
                            ...
                            <div class="p-3.5 sm:p-5">
                                {{-- Top Row: Subject badge + Status + Attachment --}}
