@extends($profileLayout ?? 'layouts.guru')

@section('title', 'Profil Saya')
@section('header', 'Profil Saya')

@section('content')
<div class="space-y-4 sm:space-y-6 pb-8 max-w-2xl mx-auto">

    {{-- Success / Error Alert --}}
    @if(session('success'))
    <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 3500)"
         x-transition.duration.500ms
         class="bg-emerald-50 border border-emerald-200 rounded-2xl px-4 py-3 flex items-center gap-3">
        <div class="w-8 h-8 bg-emerald-100 rounded-xl flex items-center justify-center shrink-0">
            <svg class="w-4 h-4 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
        </div>
        <p class="flex-1 text-xs font-bold text-emerald-700">{{ session('success') }}</p>
        ...
        <div class="px-5 pb-6 -mt-10 relative">
            <div class="flex items-end justify-between mb-4">
                ...
                <span class="px-3 py-1 bg-blue-50 text-blue-600 text-[10px] font-black uppercase tracking-widest rounded-full border border-blue-100">
                    {{ $profileRoleLabel ?? Auth::user()->roles->first()?->name ?? 'Guru' }}
                </span>
            </div>
    ...
    <div class="grid grid-cols-2 gap-3">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
            ...
            <p class="text-[9px] font-bold text-gray-400 uppercase tracking-wider">NIP</p>
            <p class="text-sm font-black text-gray-900 mt-0.5 truncate">{{ $user->nip ?? '-' }}</p>
        </div>
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
            ...
            <p class="text-sm font-black text-gray-900 mt-0.5 truncate">{{ $user->phone ?? '-' }}</p>
        </div>
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <form action="{{ route($profileUpdateRoute ?? 'guru.profile.update') }}" method="POST" class="p-5 space-y-4">
            @csrf
            <div>
            ...
            <div x-data="{ open: false }" class="pt-2">
                ...
                <div x-show="open" x-collapse x-cloak class="space-y-3 pb-1">
                    ...
                    <div>
                        <label class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5 block">Konfirmasi Password</label>
            ...
            <button type="submit"
                    class="w-full py-3 bg-linear-to-r from-blue-600 to-indigo-600 text-white rounded-xl text-xs font-black uppercase tracking-wider shadow-lg shadow-blue-500/20 hover:from-blue-700 hover:to-indigo-700 active:scale-[0.98] transition-all">
                Simpan Perubahan
            </button>
        </form>
    </div>
