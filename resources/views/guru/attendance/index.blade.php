...
@php
    ...
    if (isset($students)) {
        foreach($students as $student) {
            ...
            $initialStatusMap[$student->id] = $manualStatus ?? $resolvedStatus ?? '';
        }
    }
    $initialStats = [
        'present' => 0, 'sick' => 0, 'excused' => 0, 'late' => 0, 'absent' => 0, 'unknown' => 0
    ];
    foreach ($initialStatusMap as $status) {
        if ($status && isset($initialStats[$status])) {
            $initialStats[$status]++;
        }
    }
@endphp
<div class="space-y-4 sm:space-y-6 pb-24 sm:pb-6"
     x-data='attendanceManager(@json($initialStats))'>

    {{-- Header Description --}}
    <div>
        <p class="text-xs sm:text-sm text-gray-500 leading-relaxed">
            Kelola kehadiran siswa <strong class="text-gray-700">{{ $selectedClassId ? $classes->firstWhere('id', $selectedClassId)?->name : '—' }}</strong> hari ini.
        </p>
    </div>

    {{-- Class Selector Card --}}
    <form method="GET" action="{{ route('guru.attendance.index') }}" class="bg-white rounded-2xl shadow-sm border border-gray-100">
        <div class="p-4 sm:p-5">
            <div class="flex items-center gap-3 mb-4">
                ...
                <div class="flex-1">
                    <h3 class="text-sm font-bold text-gray-800">Pilih Kelas</h3>
                    <p class="text-[11px] text-gray-500">{{ \Carbon\Carbon::today()->translatedFormat('l, d F Y') }}</p>
                </div>
    ...
    @if($selectedClassId)
        ...
        <div class="-mx-4 sm:mx-0 px-4 sm:px-0" @update-stats.window="updateStats($event.detail)">
            <div class="flex sm:grid sm:grid-cols-5 gap-3 sm:gap-3 overflow-x-auto hide-scrollbar snap-x snap-mandatory pb-2 sm:pb-0">
        ...
        <form action="{{ route('guru.attendance.store') }}" method="POST">
            ...
            <div class="space-y-3 sm:space-y-0 sm:bg-white sm:rounded-2xl sm:shadow-sm sm:border sm:border-gray-100 sm:overflow-hidden">
                ...
                @forelse($students as $student)
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden sm:border-0 sm:shadow-none sm:rounded-none sm:border-b sm:border-gray-50 last:sm:border-b-0"
                         x-data="{
                            status: '{{ $initialStatusMap[$student->id] ?? '' }}',
                            expanded: false,
                            init() {
                        ...
                        <button type="button" @click="expanded = !expanded" class="sm:hidden w-full px-4 py-3 flex items-center gap-3 text-left active:bg-gray-50 transition-colors">
                            ...
                            <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-[10px] font-bold flex-shrink-0"
                                  :class="{
                                      ...
                                      'bg-gray-50 text-gray-400': !status
                                  }"
                                  x-text="!status ? 'Belum absen' : status === 'present' ? 'Hadir' : status === 'sick' ? 'Sakit' : status === 'excused' ? 'Izin' : status === 'late' ? 'Telat' : status === 'unknown' ? 'Perlu Verifikasi' : 'Alpha'">
                            </span>
                            {{-- Chevron --}}
                ...
                @empty
                    <div class="sm:hidden bg-white rounded-2xl shadow-sm border border-gray-100 p-10 text-center">
                        ...
                        <p class="text-sm font-bold text-gray-800">Tidak Ada Siswa</p>
                        <p class="text-xs text-gray-400 mt-1">Kelas ini belum memiliki siswa terdaftar.</p>
                    </div>
                @endforelse

                {{-- Empty Search --}}
                <div x-show="search && filteredCount === 0" x-cloak>
            ...
            {{-- Save Button (mobile: fixed bottom, desktop: inline) --}}
            <div x-show="!isLocked" x-cloak class="sm:mt-6">
                <button type="submit"
    ...
    @else
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-10 sm:p-16 text-center">
            ...
            <h3 class="text-base sm:text-lg font-bold text-gray-800">Belum Ada Kelas Dipilih</h3>
            <p class="text-xs sm:text-sm text-gray-500 mt-1 max-w-xs mx-auto">Pilih kelas terlebih dahulu untuk mulai mengisi presensi siswa.</p>
        </div>
    @endif
</div>
@endsection
        ...
        return {
            ...
            updateStats(detail) {
                if (this.stats[detail.oldStatus] !== undefined) {
                    this.stats[detail.oldStatus] = Math.max(0, this.stats[detail.oldStatus] - 1);
                }
            ...
            markAllPresent() {
                document.querySelectorAll('[x-data]').forEach(el => {
                    if (el.__x && el.__x.$data && el.__x.$data.status !== undefined) {
